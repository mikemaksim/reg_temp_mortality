...
load("initial_dataframe.RData")
 
no_of_lags = 3
seasonality_spline_df = 7
knots_percentiles = c(0.25,0.5,0.75)
...
years_list = c(2004,2008,2012,2016,2020)
...
for (region in regions_list) {
  # regional_boundary_knots = boundary_knots
  ...
  for (year_num in 1:(length(years_list)-1) ) {
    ...
    m = glm(
      ...
      SDR ~ temp_spline + ns(week, df = seasonality_spline_df * length(unique(region_mortality$Year)) ) + 
        pm10 + surface_o3,
      family = quasipoisson(),
      offset = log(Pop),
      na.action = na.omit
...
save(estimates,coef_temp,Slist,bvar,mmt, file = "first-level_estimates.RData")
